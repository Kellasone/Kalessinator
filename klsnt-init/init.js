    ...
    setTimeout(() => {
        ...
        secretXHR.onreadystatechange = async function () {
            if (this.readyState == 4 && this.status == 200) {
                ...
                kalessinator_variables.version = secretXHR.responseText.split("[\"version=")[1].split("\",")[0];
                kalessinator_variables.requiredVersion = secretXHR.responseText.split(",\"requiredVersion=")[1].split("\",")[0];

                for (let i = 0; i < scripts.length; i++) {
                    if (scripts[i].innerHTML.includes("gameVars"))
                        kalessinator_variables.userKey = scripts[i].innerHTML.split("forgeofempires.com/game/json?h=")[1].split("',")[0];
                }
...
function Signature(userKey, secret, body) {
    const data = userKey + secret + JSON.stringify(body);
